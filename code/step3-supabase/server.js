// ============================================
// server.js — 우리 반 패들렛 서버 (Supabase 모드)
...
// 환경변수 검증 — Supabase 연결 정보가 있는지 확인합니다
...
if (!process.env.SUPABASE_URL || !process.env.SUPABASE_ANON_KEY) {
  ...
  console.error('');
  console.error('   💡 해결 방법:');
  console.error('   1. 프로젝트 폴더에 .env 파일을 만드세요');
  console.error('   2. .env.example 파일을 참고해서 아래 두 값을 채워주세요:');
  console.error('      SUPABASE_URL=https://xxxxxxxxxxxxx.supabase.co');
  console.error('      SUPABASE_ANON_KEY=eyJhbGciOi...');
  console.error('');
  console.error('   Supabase 대시보드 → Settings → API 에서 확인할 수 있습니다.');
  console.error('');
  process.exit(1);
}
...
async function startServer() {
  try {
    ...
    console.error('   2. .env 파일에 SUPABASE_URL과 SUPABASE_ANON_KEY가 올바른가요?');
    console.error('   3. Supabase 프로젝트에 cards, boards 테이블을 만들었나요?');
    console.error('      (setup.sql 파일을 Supabase SQL Editor에서 실행하세요)');
    console.error('');
    process.exit(1);
  }
}
