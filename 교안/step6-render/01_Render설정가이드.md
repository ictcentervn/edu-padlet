...
환경변수를 설정해야 앱이 동작합니다. 두 가지 방법이 있습니다:

### 방법 A: 서비스 생성 시 설정
서비스 생성 화면 하단 "Environment Variables" 섹션에서 바로 추가 가능합니다.

### 방법 B: 서비스 생성 후 설정 (추천)
서비스를 먼저 만든 뒤 환경변수를 추가합니다. 빌드가 실패해도 괜찮습니다!

1. 서비스 대시보드 좌측 메뉴에서 **"Environment"** 클릭
...
4. **"Save, rebuild, and deploy"** 클릭 → 자동으로 재빌드 + 배포!

> 💡 **팁:** `.env` 파일이 있다면 **"Add from .env"** 버튼으로 한번에 추가할 수도 있습니다.

> 💡 교사 메모: 학생들이 Supabase 키를 잊어버린 경우, Supabase 대시보드 → Settings → Data API에서 다시 확인할 수 있습니다.
